...
Produces a structured description of the Noema language's interface
...
def generate_capability() -> Dict[str, Any]:
    ...
    """
    return {
        "noema_capability": CAPABILITY_SCHEMA_VERSION,
        "noema_schema": NOEMA_SCHEMA_VERSION,
        "generator": f"noema-python-{__version__}",
...
def _errors() -> List[Dict[str, Any]]:
    ...
    # test that every class in ``noema.runtime.errors`` that subclasses
    # ``NoemaError`` appears here, so drift is caught.
    return [
        {
            "name":  "NoemaError",
            "when":  "Base class for all Noema-level errors. Not raised directly.",
            "fatal": False,
        },
        {
            "name":  "ParseError",
...
def _surface_keywords() -> Dict[str, Any]:
    ...
    Included because an agent producing .nm surface text needs to know
    which spellings are accepted. An agent producing canonical JSON
    directly (the preferred path) can ignore this section.
    """
    # Built from noema.parser.tokens so the table cannot drift.
