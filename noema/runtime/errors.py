...
class RecursionLimitError(NoemaError):
    """Raised when Noema call depth exceeds the interpreter's limit.

    Exists because a Noema program is untrusted input to a host. Without an
...
class BottomPropagationError(NoemaError):
    ...
    the authoring mistake as a typed Noema error rather than a Python
    ``TypeError``.
    """
