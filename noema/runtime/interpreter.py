...
def run(
    ...
    *,
    ...
    try:
        ...
        # to a typed Noema error so the host never sees a bare
        # RecursionError.
...
class Interpreter:
    # Default maximum Noema call depth. A Noema program is untrusted input
    # to the host; this bound keeps pathological modules from exhausting
    # the Python stack. Default is conservative because each Noema frame
    ...
    def _push_depth(self) -> None:
        ...
        # limit before our Noema counter trips, map that back to a typed
        # Noema error so hosts never see a bare RecursionError.
        self._depth += 1
    ...
    def _call_primitive(self, name: str, arg_exprs: tuple, frame: _Frame) -> Any:
        ...
        try:
            ...
            # Typed Noema errors pass through — primitives that already
            # classify their failures should not be re-wrapped.
            raise
        except Exception as exc:
            # Wrap host exceptions in a typed Noema error so hosts can
