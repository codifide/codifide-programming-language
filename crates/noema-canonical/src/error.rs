...
/// Errors from parsing or serializing Noema canonical form.
#[derive(Debug)]
pub enum Error {
    /// Underlying JSON parse error.
    Json(serde_json::Error),
    /// The document was syntactically JSON but not a valid Noema canonical
    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Error::UnsupportedVersion(v) => {
                write!(f, "unsupported Noema schema version: {v:?}")
            }
        }
    }
}
