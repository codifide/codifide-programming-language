...
pub fn to_canonical_json(m: &Module) -> JsonValue {
    ...
    let mut out = Map::new();
    out.insert("noema".to_string(), JsonValue::String(SCHEMA_VERSION.to_string()));
...
pub fn from_canonical_json(v: &JsonValue) -> Result<Module, Error> {
    ...
    match obj.get("noema").and_then(|v| v.as_str()) {
        Some(s) if s == SCHEMA_VERSION => {}
        Some(s) => return Err(Error::UnsupportedVersion(s.to_string())),
        None => return Err(shape("missing `noema` version tag")),
    }
    let name = obj
        .get("module")
        .and_then(|v| v.as_str())
