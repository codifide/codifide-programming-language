...
//! user-defined function. Pure expressions (Lit, Ref, Attr on a local,
//! Concat of literals) are not worth spawning threads for.

use std::collections::HashSet;
use rayon::prelude::*;

use codifide_canonical::ast::{Expr, Module};
...
/// This is a static over-approximation: it includes effects from all
/// branches of an `If`, even though only one branch evaluates at runtime.
/// Conservative is correct — it may serialize expressions that could
/// theoretically run in parallel, but never runs two expressions in
/// parallel when they shouldn't be.
pub fn expr_effects(expr: &Expr, module: &Module) -> HashSet<String> {
    let mut out = HashSet::new();
    collect_effects(expr, module, &mut out);
    out
}

fn collect_effects(expr: &Expr, module: &Module, out: &mut HashSet<String>) {
    match expr {
        Expr::Call { fn_, args } => {
            // User-defined function: use its declared signature effects.
            ...
            // This is conservative: a pure primitive call won't block
            // parallelism, and an effectful primitive call inside a
            // user-defined function is covered by the function's declared
            // effects above.
            for a in args { collect_effects(a, module, out); }
        }
        Expr::Bind { expr: e, body, .. } => {
            collect_effects(e, module, out);
            collect_effects(body, module, out);
        }
        Expr::Seq { steps } => {
            for s in steps { collect_effects(s, module, out); }
        }
        Expr::Concat { parts } => {
            for p in parts { collect_effects(p, module, out); }
        }
        Expr::Believe { subject, arms, otherwise } => {
            collect_effects(subject, module, out);
            for (c, v) in arms { collect_effects(c, module, out); collect_effects(v, module, out); }
            collect_effects(otherwise, module, out);
        }
        Expr::Attr { target, .. } => { collect_effects(target, module, out); }
        Expr::If { cond, then_, else_ } => {
            collect_effects(cond, module, out);
            collect_effects(then_, module, out);
            collect_effects(else_, module, out);
        }
...
pub fn all_disjoint(exprs: &[&Expr], module: &Module) -> bool {
    if exprs.len() < 2 { return false; }
    let effect_sets: Vec<HashSet<String>> = exprs.iter()
        .map(|e| expr_effects(e, module))
        .collect();
...
/// Does `expr` contain a `Call` to a user-defined function?
/// This is the threshold check (PE-4): pure expressions are not worth
/// spawning threads for.
pub fn contains_user_call(expr: &Expr, module: &Module) -> bool {
    match expr {
        Expr::Call { fn_, args } => {
            if module.symbols.iter().any(|(n, _)| n == fn_) {
                return true;
            }
            args.iter().any(|a| contains_user_call(a, module))
        }
        Expr::Bind { expr: e, body, .. } => {
            contains_user_call(e, module) || contains_user_call(body, module)
        }
        Expr::Seq { steps } => steps.iter().any(|s| contains_user_call(s, module)),
        Expr::Concat { parts } => parts.iter().any(|p| contains_user_call(p, module)),
        Expr::Believe { subject, arms, otherwise } => {
            contains_user_call(subject, module)
                || arms.iter().any(|(c, v)| contains_user_call(c, module) || contains_user_call(v, module))
                || contains_user_call(otherwise, module)
        }
        Expr::Attr { target, .. } => contains_user_call(target, module),
        Expr::If { cond, then_, else_ } => {
            contains_user_call(cond, module)
                || contains_user_call(then_, module)
                || contains_user_call(else_, module)
        }
        _ => false,
    }
}
...
///    This is the key threshold (PE-4): `list(f(1), f(2), f(3))` qualifies;
///    `walk(s, add(i, 1), step(s, i, d))` does not (mixed args).
///    Mixed-arg calls are typically recursive patterns where thread-spawn
///    overhead would dominate.
/// 3. All pairs have disjoint effect sets.
pub fn should_parallelize(exprs: &[&Expr], module: &Module) -> bool {
    if exprs.len() < 2 { return false; }
    // All args must be direct user calls — no mixed arithmetic/simple args.
    if !exprs.iter().all(|e| is_direct_user_call(e, module)) { return false; }
    all_disjoint(exprs, module)
}

/// Is `expr` a direct `Call` node (not nested) to a user-defined function?
fn is_direct_user_call(expr: &Expr, module: &Module) -> bool {
    match expr {
        Expr::Call { fn_, .. } => module.symbols.iter().any(|(n, _)| n == fn_),
        _ => false,
    }
}
